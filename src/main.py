...
from src.state.persistence import save_graph, get_db_path
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=["full", "recon-only"], default="full", help="Operation mode")
    parser.add_argument("--no-router", action="store_true", help="Ablation: Disable LLM router, force local")
    parser.add_argument("--no-graph", action="store_true", help="Ablation: Disable state graph")
    ...
    logger.info("Starting PenAgent", extra={"target": args.target, "mode": args.mode, "run_id": run_id})
    ...
    try:
        ...
        logger.info("Graph execution completed", extra={"steps": final_state["step_count"]})
        
        # Save persistence
        db_path = get_db_path(args.target, str(int(time.time())))
        save_graph(final_state["attack_graph"], db_path)
